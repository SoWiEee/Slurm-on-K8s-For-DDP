...
| 類別 | 議題 | 嚴重度 | 章節 |
|---|---|:---:|---|
| 程式碼結構 | ~~`operator/app.py` 906 LoC 單檔；`_process_pool` 178 行~~ → **✅ v5 已拆成 5 模組**（app.py 353 LoC + watchers/reconciler/scale_actions/fragmentation_loop） | — | §1.2 |
| 死碼 | ~~`services/rl_scheduler/{ppo_*,hierarchical,snapshot_agent}.py` 已被 ruff exclude，但仍佔 ~1500 LoC~~ → **✅ v5 已 git rm** | — | §1.3 |
| 客製排程脆弱性 | 4 個 score factor + 3-arm UCB1 + 4 條 submit-path 網路呼叫，**飽和條件下未驗證** | 🟠 P1 | §3 |
| 目標差距 | DRL 在 sim 仍輸 score baseline 42–386%（v3 更差） | 🟠 P1 | §4.3 |

**v5 review 實作完成項目**（依優先序執行於 commit `<see git log>`）：

| 編號 | 項目 | 內容 |
|---|---|---|
| **C3** | 刪除死碼 | 移除 `hierarchical.py` / `ppo_train.py` / `ppo_masked_train.py` / `snapshot_agent.py` / `eval_hierarchical.py`（~1040 LoC）；清空 ruff.toml exclude 列表 |
| **K1** | resource limits | operator / controller / login 全部補上 Guaranteed-QoS requests/limits；values.yaml 暴露 `operator.resources` / `slurm.controllerResources` / `login.resources` 三個欄位 |
| **R19** | submit helper | 新 `chart/lua/submit_helper.lua` + `tests/lua/submit_helper_test.lua`（26 個測試 PASS）；auto-fill `--mem` / `--partition` / `--qos`，全部 silent fallback；values.yaml 新增 `slurm.jobSubmit.helper` 配置；CI lua-test workflow 已加入 |
| **R20** | per-job dashboard | 新 `chart/dashboards/per-job-gpu.json`（uid `slurm-k8s-per-job-gpu`）；$hostname × $gpu × $job_id template variables；包含 DCGM 5 個 panel + provisioning latency exemplar 連 Tempo + TraceQL deep-link |
| **C2** | 拆 operator/app.py | 906 LoC → 353 LoC，依 mixin pattern 分到 `watchers.py` (118) / `reconciler.py` (210) / `scale_actions.py` (234) / `fragmentation_loop.py` (118)；25 個 operator tests 全過 |
...
### 1.2 結構問題：`operator/app.py` 過於龐大 — ✅ v5 已修

**修法摘要：** 採用 mixin pattern 把 906 LoC 拆成 5 個檔案，每個檔 < 250 LoC：

```text
operator/
  app.py              353 LoC  ← OperatorApp __init__/run + _PoolEventQueue + JsonLogger + StatefulSetActuator
  watchers.py         118 LoC  ← WatcherMixin: _watch_statefulsets/_watch_pods/_poll_slurm_state/_periodic_timer
  reconciler.py       210 LoC  ← ReconcilerMixin: _process_pool（含 OTel job lifecycle 邏輯）
  scale_actions.py    234 LoC  ← ScaleActionsMixin: _do_scale_up/_do_scale_down/_do_keep
  fragmentation_loop.py  118 LoC  ← FragmentationLoopMixin: _fragmentation_loop/_fragmentation_tick
```

`OperatorApp(WatcherMixin, ReconcilerMixin, ScaleActionsMixin, FragmentationLoopMixin)` 繼承四個 mixin，所有方法用 `self.*` 存取共享狀態——零行為改變，25 個 operator 單元測試全過。

**後續仍可加強**：scale-up/down 邏輯目前仍純靠 e2e 驗證；可考慮抽出 fake-K8s-client fixture 補單元測試（記在 backlog，非阻擋項）。

### 1.3 死碼：`services/rl_scheduler/` 中已淘汰的模組 — ✅ v5 已刪

v5 review 已 `git rm` 以下 5 個檔案（合計約 1040 LoC）並清空 `ruff.toml` 的 exclude 列表：

```
services/rl_scheduler/hierarchical.py        — D-LinUCB outer loop（M10 後改用 flat DSAC + RLPD）
services/rl_scheduler/ppo_train.py           — 早期 PPO 版本（被 DSAC 取代）
services/rl_scheduler/ppo_masked_train.py    — PPO 加 action mask
services/rl_scheduler/snapshot_agent.py      — 早期 snapshot interface
eval/scripts/eval_hierarchical.py            — 對應的 eval script
```

歷史版本仍可透過 `git log --all -- services/rl_scheduler/hierarchical.py` 找到。`docs/scheduler.md` 對 hierarchical 的引用已標記為 `archived 2026-05`。
...
```text
sim_train.py / sim 環境
  -> 訓練 DSAC policy，輸出 runs/.../train/dsac.pt
  -> docker/rl-scheduler image COPY dsac.pt
  -> Helm 部署 rl-scheduler Deployment
  -> job_submit.lua 在 sbatch submit path 計算 score baseline
  -> job_submit.lua 呼叫 rl-scheduler /decide
  -> DSAC 回傳 priority_boost，live cluster 依 Slurm priority 排程
```

因此 S2 的風險不是「DSAC 不能上 live」，而是 **sim / 評估 / fallback 所依賴的 score 參考實作** 與 **live Lua 實際執行的 submit score** 可能在後續調參時漂移。若 drift 發生，訓練與評估報告會以為某個 score baseline 有效，但 live cluster 實際使用的是另一套分數。

**已採用修法 A：** 新增 `sim.scheduler.score.submit_score_factors()` 作為 live submit score 的 Python 參考實作，並新增 `tests/test_score_parity.py`。測試會用 Helm render 出真正的 `job_submit.lua`，再用 Lua interpreter 執行 `compute_score()`，逐 case 比對 Python reference 的 `score / mps_fit / vram_fit / topology / fragmentation / pred_runtime`。CI 由 `.github/workflows/score-parity-test.yml` 執行，能在 chart、values 或 Python score 變更時阻止 drift。
...
| # | 議題 | 類別 | 嚴重度 | 工程量 | 狀態 |
|---|---|---|:---:|:---:|:---:|
| C1 | 補 `services/rl_scheduler/` 單元測試（serve/per/replay buffer） | 測試覆蓋 | 🟠 P1 | 1 天 | ✅ |
| K1 | operator/controller/login 補 resources requests/limits | K8s | 🟠 P1 | 10 行 yaml × 3 | ✅ v5 |
...
| # | 議題 | 類別 | 嚴重度 | 工程量 | 狀態 |
|---|---|---|:---:|:---:|:---:|
| C2 | 拆 `operator/app.py`（906→5×~200） | 程式碼結構 | 🟡 P2 | 2 天 | ✅ v5 |
| C3 | 刪除 `hierarchical.py` / `ppo_*.py` / `snapshot_agent.py` 死碼 | 程式碼乾淨 | 🟡 P2 | 10 分鐘 | ✅ v5 |
...
| R11 | Login pod resource limit（已併入 K1） | — | — | — | ✅ v5 |
...
| ★R19 | submit helper 補 `--mem` / partition / qos | 易用性 | ★ | 中 | ✅ v5 |
| ★R20 | Per-job × per-GPU × full-lifecycle dashboard | 視覺化 | ★ | 中 | ✅ v5 |
...
Week 1 (NOW)：F1 — 重新組織 thesis 敘事框架（平台/OTel/score function 三大特色）
              [C2/C3/K1/R19/R20 已於 v5 commit 完成 ✅]
...
Week 4     ：F2 — score-residual RL ablation 實驗（可選）
              R20 升級 — per-job dashboard 補上實機截圖
