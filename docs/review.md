...
**改進方向：** Slurm `--exclusive` 搭配 job array，或整合 Volcano scheduler。
...
**改進方向：** 加入 exponential backoff；readinessProbe 用 `/healthz` HTTP endpoint 或 lock file exec。
...
| **P3** | NetworkPolicy Egress 規則 | 安全 | 中 |
