...
### 補強：verify 不可把 FAILED 當成功

你最新貼的輸出其實已顯示：

- `JobState=FAILED Reason=NonZeroExitCode`（job 7 / job 8）

但舊版 verify 邏輯只「等待 job 離開 queue」就往下跑，最後仍印出 `phase3 verify completed`，這是明顯誤判。

已修正為：

1. 每個 job 完成後強制檢查 `JobState` 與 `ExitCode`。
2. 若不是 `COMPLETED` + `0:0`，立即 `exit 1`。
3. 失敗時額外 tail `slurm-<jobid>.out`，讓 root cause 更快定位。


### 補強：多節點 job 前先確保節點容量與 Slurm 可見狀態

你這次 job 10 的 `FAILED ExitCode=244:0`，從症狀看是多節點 step 啟動時，第二節點尚未完全就緒/可用造成的常見失敗型態。

修正策略：

1. verify 在送 MPI/Torch 前，先把 `slurm-worker` scale 到至少 2（可用 `VERIFY_MIN_WORKER_REPLICAS` 調整）。
2. 再等 Slurm `sinfo -N` 至少看到 2 個可用節點（`idle/mix/alloc`）才送 job。
3. 若等不到，直接輸出 `sinfo -R` + `scontrol show nodes` 並失敗，避免不必要的 job 重試。


### 補強：如何判定 MPI「真的有跨節點成功執行」

你問的重點很正確：只看 `sbatch` 成功或 job 離隊列，不足以證明 MPI 測試成功。

本次補上兩個判定層：

1. **控制面健康檢查（送單前）**
   - 確認 worker pod 內 `munged` + `slurmd` 都在跑。
   - 對 `DOWN/NOT_RESPONDING` 節點嘗試 `State=RESUME`。

2. **資料面結果檢查（job 完成後）**
   - 從 `slurm-<jobid>.out` 擷取 `rank-host=`。
   - 要求至少 2 個不同 host，否則判定 MPI smoke 失敗。

所以現在可用以下標準判斷 MPI 是否正確：
- `JobState=COMPLETED` 且 `ExitCode=0:0`
- `rank-host` 至少兩個不同節點


### 補強：避免 rollout 期間固定 Pod 名稱檢查造成誤判

你這次看到：

- `cannot exec into a container in a completed pod; current phase is Succeeded`

這代表 verify 用固定 pod 名稱（例如 `slurm-worker-1`）做 `kubectl exec` 時，剛好打到 rollout 過程中的 completed/替換時點。

修正為：

1. 先動態列出目前 `Running` 的 `app=slurm-worker` pods。
2. 取前 N 個（N = `VERIFY_MIN_WORKER_REPLICAS`）做 `munged/slurmd` 健康檢查。
3. 若 Running pods 不足，直接顯示 pod 列表並失敗。

這樣可避免固定命名在 StatefulSet 更新窗口造成偽失敗。


### 補強：verify 期間暫停 Operator，避免 worker 立刻被縮容

你這次的關鍵現象：

- `slurm-worker-1` 剛 `Running` 就被 `Terminating/Completed`
- verify 在 daemon health check 時剛好 hit 到不存在或已完成 pod

這多半是 Phase2 operator 在你送 job 前先判定可縮容，導致 verify 與 autoscaler 互相打架。

修正方式：

1. verify 開始時先暫停 `slurm-elastic-operator`（scale to 0）。
2. verify 結束（成功/失敗）都透過 `trap` 自動恢復原 replicas。
3. 如需保留 operator 行為可設 `DISABLE_OPERATOR_DURING_VERIFY=false`。

另外，daemon 檢查加了短重試，降低 rollout 窗口瞬時 NotFound 的誤判。


### 補強：Worker "Running" 不等於準備好，需看 READY=1/1

你觀察到 `slurm-worker-1` 會短暫 `Running` 後 `Terminating/Completed`，這就是典型「Pod 生命週期抖動」。
...
本次 verify 補上等待邏輯：若未達到至少 N 個 `1/1 Running` worker（預設 N=2）就不進入 MPI 檢查。

另外 operator 暫停/恢復過程中，舊 pod 出現 `Terminating/Error` 是 deployment rolling 常見現象；只要新 pod 最終 `Running` 即可。


### 補強：MPI 驗證輸出路徑改為動態解析 StdOut

你這次的症狀是：

- `JobState=COMPLETED` 但 `mpi output validation ... unique_hosts=0`

這通常不是 MPI 沒印出，而是 verify 去讀錯輸出檔路徑（先前假設 `/root/slurm-<jobid>.out`）。

修正為：

1. 先用 `scontrol show job -o` 解析 `StdOut=` 真實路徑。
2. 再從該路徑解析 `rank-host=`。
3. 保留 `/root/slurm-<jobid>.out` 作 fallback 與除錯 tail。

這可避免不同工作目錄或 Slurm 設定導致的誤判。


### 補強：MPI `rank-host` 解析同時掃描 StdOut / StdErr 與相對路徑

你這次仍出現：

- `JobState=COMPLETED ExitCode=0:0`
- 但 `mpi output validation ... unique_hosts=0`

代表資料面解析仍可能漏讀檔案，而不是 MPI 一定沒跑。

修正：

1. 解析 `scontrol show job -o` 的 `StdOut` 與 `StdErr`。
2. 同時嘗試路徑變體：原始路徑、`/root/<relative>`、`/<relative>`。
3. 用 `grep -Eo 'rank-host=...'` 做寬鬆匹配（不再要求行首開頭）。

這可涵蓋不同 Slurm 輸出路徑與格式差異，減少 `unique_hosts=0` 假陰性。


### 補強：MPI 解析失敗時的 debug 與 allocation-based fallback
...
這代表「輸出檔抓取/解析」可能落空，但 Slurm 作業本身其實成功。

本次補強：

1. 驗證失敗時先列出候選路徑（StdOut/StdErr/WorkDir 變體）與存在檔案 tail。
2. 額外輸出 `scontrol show job -o` 的 `NumNodes/NodeList`。
3. 若 `NumNodes>=2`，以 allocation evidence 作 fallback pass（針對 MPI-like smoke）。

這樣可避免輸出解析問題造成假陰性，同時保留完整 debug 資訊。
