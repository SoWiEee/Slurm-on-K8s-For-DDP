...
## Phase 3 補充：Windows 上 CSI NFS 安裝 `evalsymlink failure` 修正

### 症狀

執行 `phase3/scripts/bootstrap-phase3.sh` 時，安裝 CSI NFS driver 失敗：

- `error: evalsymlink failure on ... kustomize-xxxx/deploy/kubernetes/overlays/stable`

### 原因分析

`kubectl apply -k github.com/...` 會先把遠端 repo 拉到本機暫存後再跑 kustomize。
在 Windows 某些環境（路徑轉換/符號連結處理）下，暫存目錄中的 overlay 解析可能失敗。

### 修正策略

在 `bootstrap-phase3.sh` 加入雙路徑安裝：

1. 先嘗試原本 remote kustomize：

```bash
kubectl apply -k github.com/kubernetes-csi/csi-driver-nfs/...\n```

2. 若失敗，自動 fallback：
   - `git clone --branch <ref>` 到本地暫存目錄
   - 改用本地路徑 `kubectl apply -k <repo>/deploy/kubernetes/overlays/stable`

這個方式可繞過 Windows 上遠端 kustomize 暫存與 symlink 的相容性問題。


### 二次修正（local clone 仍失敗）

在某些 Windows 環境，即使已 fallback 到 local clone，`deploy` 可能不是可追蹤的目錄（例如 symlink 在 checkout 後退化為文字檔），導致：

- `not a valid directory: .../deploy/kubernetes/overlays/stable`

因此 `bootstrap-phase3.sh` 再補強：

1. 優先用標準路徑 `deploy/kubernetes/overlays/stable`。
2. 若 `deploy` 是檔案，嘗試讀取其 target 並拼出 overlay 路徑。
3. 最後掃描 repo 中 `*/kubernetes/overlays/stable`，自動選可用路徑。

這讓 fallback 不只「改成本地 clone」，也能處理 clone 後 symlink 行為差異。
