...
## M11：Score function 後續優化

已盤點的優化項目如下：

| # | 優化點 | 工期 | 狀態 |
|---|---|---|:---:|
| M11-1 | `horizon` auto-tune（predictor 端從 sacct p50 算出推薦值） | 0.5 天 | ⬜ |
| M11-2 | 公式刪掉 γ topology 因子（從未啟用） | 0.5 天 | ⬜ |
| M11-4 | `f_mps_fit` 公式翻轉方向（修跟 fragmentation 重複的 bug） | 1 天 | ⬜ |
| M11-6 | `f_runtime_short` 加 age boost 防 SJF 餓死長 job | 0.5 天 | ⬜ |
| M11-10 | M7 改 suspend+resume 取代 full requeue | 1–2 週 | ⬜ |
| M11-11 | Predictor 改 quantile regression（p10/p50/p90） | 1 週 | ⬜ |

跳號的 3、5、7、8、9 屬於低 ROI 清理或受限於 cluster 規模做不到的項目，留在 `docs/scheduler-score-spec.md` §9 附近備查不單獨列 milestone。

### Sprint 1（M11-1 / 2 / 4 / 6）— 公式清理
...
### Sprint 2（M11-10）— M7 suspend+resume
...
### Sprint 3（M11-11）— Predictor quantile regression
