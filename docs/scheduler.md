...
- #1 E7 live cluster：跑了 20-job × 3 pass paired comparison（vendor / our M3 / our_pred M3+M5）。Headline 是 our_pred 比 vendor 快 −57.75%、20/20 改善。但 our_pred vs our 只差 0.13% — predictor 在 homogeneous 20-job workload 上沒帶來邊際改善（同 user、同 gpu_type、submit_ts 集中，predictor 對組內排序沒信號）。這跟 sim E4 vs E3 paired −18% 形成有趣對比：predictor 的價值跟 workload heterogeneity 直接相關。詳見 `docs/eval-writeup.md` §7、§7.6。
