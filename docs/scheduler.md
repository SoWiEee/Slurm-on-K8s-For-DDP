...
**M11-1 Horizon auto-tune**：predictor service retrain 跑完後從訓練 trace 算 p50 runtime，service `/recommended_horizon` endpoint 回值；lua 啟動時拉一次當預設。對應 `docs/scheduler-score-spec.md` §9 條件 3 的自動化。

**M11-2 刪 γ**：公式從 5 因子變 4 因子，chart values `scoreWeights.topology` 標 deprecated。Spec §3.3 改成 historical note。

**M11-4 修 `f_mps_fit` 方向**：當前 `1 - residual_mps/100` 跟 `f_fragmentation` 是同方向（兩個都懲罰 unbalance），double-counting。改成 `mps_req / best_node.mps_free`（用了越大比例越高分），小 job 跟大 job 分數方向不再矛盾。**這個改動同時是 Tetris-style packing 的啟發**，參考：
...
**M11-6 Age boost**：`f_runtime_short = max(base_f, age_hours / max_age_hours)`，等夠久的長 job 自動視為「短 job」插隊。經典 CPU scheduling 的 aging 反 starvation 技巧，參考：
...
- M10（Deep RL scheduler）優先做。Sim PPO + RLPD Sim2Real + 內含 D-LinUCB outer loop
- M11（公式清理 + M7 suspend+resume + predictor quantile）延到 M11 完成後再回頭，部分項可能被 DRL policy 直接學會、變成 optional
