...
- #1 E7 live cluster：跑了 20-job × 2 pass paired comparison（vendor multifactor vs M3 score）。Headline 是 −57.7% mean JCT 但帶 cluster freeze artefact；乾淨子集（小 job、cluster 穩定區段）是 −38.6%。詳見 `docs/eval-writeup.md` §7。注意這個 live 對應 sim 的 E3，不是 E4，因為 predictor service 還沒部署。
