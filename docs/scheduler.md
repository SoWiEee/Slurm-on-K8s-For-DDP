...
LinUCB 已實作（`services/weight_tuner/`），跑了 sim-only 比較。Headline 數字：

| Policy | eval mean JCT (h) | sim runs 用了 |
|---|---:|---:|
| random | 3.217 | 120 |
| **UCB1** | **2.587** | 120 |
| LinUCB | 2.745 | 120 |
...
關鍵發現：oracle 跟 static-best 只差 2.5%，所以 contextual tuning 在這個 workload mix 上沒有 headroom；LinUCB 反而被 ridge regression fit noise 拖到比 UCB1 還差。**UCB1 用 1/3 的 sim 預算拿到接近 M8 grid-best 的答案**，這才是 M9 的真正貢獻。

PPO 沒做。理由：原 spec 設想 PPO 比 LinUCB 強，但 LinUCB 結果已經證明 oracle ceiling 只多 2.5%，PPO 不會超過 oracle，預期收益 < 5%，跟驗收門檻不過分。Sim-only 已能回答「動態 tuning 值不值得」、沒必要接到 chart。

完整 writeup 在 `docs/eval-writeup.md` §8。

## M10：Score function 後續優化（規劃中，2026-05-12 起）

M8/M9 跑完後重新審視 `docs/scheduler-score-spec.md`，列出 11 個可優化點按 ROI 排序。下面把該做的分成「下個 sprint 動」跟「Future work」兩段，每項都附對應參考文獻。

### 已盤點的優化項目

| # | 優化點 | 工期 | 狀態 |
|---|---|---|:---:|
| M10-1 | `horizon` auto-tune（predictor 端從 sacct p50 算出推薦值） | 0.5 天 | ⬜ |
| M10-2 | 公式刪掉 γ topology 因子（從未啟用） | 0.5 天 | ⬜ |
| M10-4 | `f_mps_fit` 公式翻轉方向（修跟 fragmentation 重複的 bug） | 1 天 | ⬜ |
| M10-6 | `f_runtime_short` 加 age boost 防 SJF 餓死長 job | 0.5 天 | ⬜ |
| M10-10 | M7 改 suspend+resume 取代 full requeue | 1–2 週 | ⬜ |
| M10-11 | Predictor 改 quantile regression（p10/p50/p90） | 1 週 | ⬜ |

跳號的 3、5、7、8、9 屬於低 ROI 清理或受限於 cluster 規模做不到的項目，留在 `docs/scheduler-score-spec.md` §9 附近備查不單獨列 milestone。

### Sprint 1（M10-1 / 2 / 4 / 6）— 公式清理

四個一起做、約 2–3 天，會把公式從「能跑」變「乾淨可解釋」，論文 §3 系統設計章節敘事順很多。

**M10-1 Horizon auto-tune**：predictor service retrain 跑完後從訓練 trace 算 p50 runtime，service `/recommended_horizon` endpoint 回值；lua 啟動時拉一次當預設。對應 `docs/scheduler-score-spec.md` §9 條件 3 的自動化。

**M10-2 刪 γ**：公式從 5 因子變 4 因子，chart values `scoreWeights.topology` 標 deprecated。Spec §3.3 改成 historical note。

**M10-4 修 `f_mps_fit` 方向**：當前 `1 - residual_mps/100` 跟 `f_fragmentation` 是同方向（兩個都懲罰 unbalance），double-counting。改成 `mps_req / best_node.mps_free`（用了越大比例越高分），小 job 跟大 job 分數方向不再矛盾。**這個改動同時是 Tetris-style packing 的啟發**，參考：

- Grandl et al., "Multi-Resource Packing for Cluster Schedulers", **SIGCOMM 2014** — Tetris 論文，bin-packing heuristic 對 multi-resource cluster scheduling 的奠基工作。

**M10-6 Age boost**：`f_runtime_short = max(base_f, age_hours / max_age_hours)`，等夠久的長 job 自動視為「短 job」插隊。經典 CPU scheduling 的 aging 反 starvation 技巧，參考：

- Silberschatz, Galvin, Gagne, *Operating System Concepts*（OS 教科書），Chapter 5 CPU Scheduling — aging mechanism 的標準論述。
- 跟 Slurm 內建 `PriorityWeightAge` 不一樣的點：Slurm 是 priority 上加分；M10-6 是直接讓 `f_runtime_short` 飽和到 1.0，蓋過 ε 權重小的問題。

### Sprint 2（M10-10）— M7 suspend+resume

把 M8 跑出來的 negative result 翻盤的研究級工作。M7 victim 從 zero restart 改成 GPU memory checkpoint + resume，把 lost progress 從 100% 降到接近 0。

實作路徑：先在 sim 加 cost model（`--frag-resume-cost`，比 reload 高一點但遠小於 lost progress）跑 ablation 確認預期收益；再決定要不要做真實 CRIU 整合。對應參考：

- Xiao et al., "Gandiva: Introspective Cluster Scheduling for Deep Learning", **OSDI 2018** — DL job migration + suspend/resume 的基礎工作，M7 原本就是「Gandiva-lite」。
- Mahajan et al., "Themis: Fair and Efficient GPU Cluster Scheduling", **NSDI 2020** — preemption + fair-share，可借「lease-based」概念給 victim selection 加 elapsed-progress penalty。
- Berg et al., "CRIU: Checkpoint/Restore In Userspace" — 系統工具層，做 Linux process state snapshot；GPU 記憶體要搭 NVIDIA cuCheckpoint API（CUDA 12.0+，2023 出）。
- Qiao et al., "Pollux: Co-adaptive Cluster Scheduling for Goodput-Optimized Deep Learning", **OSDI 2021** — DL-aware scheduling 把 job 狀態（throughput, batch size）納入決策，可參考 victim selection 怎麼避免踢「快跑完的 job」。

### Sprint 3（M10-11）— Predictor quantile regression

當前 predictor 只回 point estimate `pred_seconds`。改用 quantile regression 訓三個分位（p10、p50、p90），lua 用 spread `(p90 - p10) / p50` 算 confidence weighting：模型對某 job 不確定（spread 大）時自動降低 `f_runtime_short` 的影響權重。對應參考：

- Koenker & Bassett, "Regression Quantiles", **Econometrica 46(1), 1978** — quantile regression 的奠基論文。
- Meinshausen, "Quantile Regression Forests", **JMLR 7, 2006** — Tree-based quantile regression，LightGBM 直接 support 的 objective。
- Tumanov et al., "TetriSched: global rescheduling with adaptive plan-ahead in dynamic heterogeneous clusters", **EuroSys 2016** — 排程時用 runtime 預測 + uncertainty 做 deadline 排程，是 confidence-weighted scheduling 在 cluster manager 上的應用範本。

### 期望結論

跑完 M10 後對 thesis 章節的影響：

- 公式從「5 因子（其中 1 個沒用）」變「4 因子，每個都跑」+ 兩個改善（mps_fit 方向、age boost）→ §2 公式表更乾淨、可解釋性提升、爭議點少。
