...
| 2026-05-07 | M6 plumbing (predictor enabled, applyTimeLimit=true) | offline E2E (lua + curl + uvicorn) | n/a | lua → predictor round-trip 完整跑通；live mode 把 `time_limit=0` 改成 10 min（pred≈620 s），fallback mode（service 殺掉）保持 0；都是 `slurm.SUCCESS`。bf_rate 改善留給 M8。 |

(M7+ entries appended here as evaluation runs.)
