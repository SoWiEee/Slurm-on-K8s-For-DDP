...
install_nfs_csi_driver() {
  ...
  git clone --depth 1 --branch "$NFS_CSI_REF" https://github.com/kubernetes-csi/csi-driver-nfs.git "$repo_dir"
  kubectl apply -k "$repo_dir/deploy/kubernetes/overlays/stable"
}
