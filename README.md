...
> 若你在 Windows 上看到 `evalsymlink failure ... kustomize ... overlays/stable`，腳本已內建 fallback：會改走 `git clone` 後本地路徑安裝，並自動偵測 overlay 目錄（含 symlink 退化成檔案的情況），避免遠端/本地 kustomize 路徑相容性問題。
