"""M11 Phase C-1: MaskablePPO training with action masking.

Uses sb3-contrib's MaskablePPO so the policy never spends probability mass
on illegal allocations. The env exposes ``action_masks()`` which returns a
bool array over Discrete(K+1) — true for jobs that can fit + no-op.

Run:
    .venv-m11/bin/python -m services.rl_scheduler.ppo_masked_train \\
        --total-steps 200000 --n-envs 4 --n-jobs 100
"""
...
def eval_policy_masked(model, env_thunk, n_episodes: int = 3) -> float:
    jcts = []
    for _ in range(n_episodes):
        ...
        if "avg_jct" in info and not np.isnan(info["avg_jct"]):
...
class EvalAgainstScoreCallback(BaseCallback):
    def __init__(self, *, eval_freq: int, eval_kwargs: dict, log_path: str,
        ...
        with open(self.log_path, "w", newline="") as fh:
            csv.writer(fh).writerow(["step", "ppo_avg_jct", "score_avg_jct", "ratio"])
    ...
    def _on_step(self) -> bool:
        ...
        ppo_jct = eval_policy_masked(self.model, eval_thunk, n_episodes=1)
        ratio = ppo_jct / self._score_jct if self._score_jct > 0 else float("nan")
        if self.verbose:
            print(f"  [eval@{self.num_timesteps}] mppo_jct={ppo_jct:.1f}  "
                  f"score_jct={self._score_jct:.1f}  ratio={ratio:.3f}")
        with open(self.log_path, "a", newline="") as fh:
            csv.writer(fh).writerow([self.num_timesteps, ppo_jct,
                                     self._score_jct, ratio])
        self.logger.record("eval/mppo_avg_jct", ppo_jct)
        self.logger.record("eval/score_avg_jct", self._score_jct)
        self.logger.record("eval/ratio_mppo_over_score", ratio)
        return True
...
def main(argv=None) -> int:
    ...
    out_dir = Path(args.out_dir or f"runs/m11_mppo_{ts}")
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"output dir: {out_dir}")

    device = "cpu"  # MlpPolicy on small obs runs faster on CPU per SB3
    print(f"torch={torch.__version__} device={device}")

    max_steps = args.n_jobs * 100
    env_fns = [
    ...
    # Marker so eval_paired can pick the right class
    (out_dir / "MASKED").write_text("1\n")
